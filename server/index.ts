...
function createSarvamWS(meetingId: string, socketId: string, speakerName: string, speakerImage: string | null, broadcastSegment: (segment: any) => void) {
...
io.on('connection', (socket: any) => {
    ...
    socket.on('join_room', async ({ meetingId, name, profileImage }: any) => {
        ...
        if (session && session.active) {
            const broadcastSegment = (segment: any) => io.to(`meeting:${meetingId}`).emit('transcript_update', segment);
